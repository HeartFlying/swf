...
| 文档名称 | 主工作流程文档（优化版）                      |
| 文档版本 | 3.0                               |
| 最后更新 | 2026-03-28（新增S5架构设计、S6详细设计阶段，Skill总数扩展至23个） |
| 适用范围 | 用户需求分析工作流                         |
...
graph TD
    ...
    B --> C[23 个 Skill 技能库<br/>内嵌校验 + 用户交互<br/>S0: Plan 制定 → S1: 需求边界 → S2: 市场校验 → S3: 技术选型 → S4: 需求整合 → S5: 架构设计 → S6: 详细设计<br/>每个 Skill 包含：前置校验 → 执行 → 用户交互 → 输出 → 用户评审确认]
...
| S101 | 需求边界界定  | S1 | ✅ S101 评审  | 执行       |
| S102 | 显性需求提取  | S1 | ✅ S102 评审  | 执行       |
| S103 | 隐性需求挖掘  | S1 | ✅ S103 评审  | **快速模式** |
| S104 | 需求验证    | S1 | ✅ S104 评审  | 执行       |
| S201 | 竞品分析    | S2 | ✅ S201 评审  | **跳过**   |
| S202 | 市场痛点验证  | S2 | ✅ S202 评审 | **跳过**   |
| S301 | 需求风险识别  | S3 | ✅ S301 评审  | 执行       |
| S302 | 技术可行性评估 | S3 | ✅ S302 评审 | 执行       |
| S303 | 技术选型 | S3 | ✅ S303 评审 | 执行       |
| S401 | 需求分类梳理  | S4 | ✅ S401 评审  | 执行       |
| S402 | 需求优先级排序 | S4 | ✅ S402 评审  | 执行       |
| S403 | 核心需求提炼  | S4 | ✅ S403 评审  | 执行       |
| S404 | 原型设计    | S4 | ✅ S404 评审 | **跳过**   |
...
- **跳过的 Skill**：S201（竞品分析）、S202（市场验证）、S404（原型设计）
- **快速模式的 Skill**：S103（隐性需求挖掘）- 执行简化版，基于显性需求快速推导 3-5 个核心隐性需求
