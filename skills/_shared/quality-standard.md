...
> **引用路径**：`../_shared/quality-standard.md`
...
| S101-S102 | 包含竞品分析维度、市场验证方法 |
| S201-S204 | 包含需求边界定义、显式/隐式需求列表、验证规则 |
| S301-S304 | 包含风险分类、可行性评估维度、技术选型标准、非功能需求 |
| S401-S406 | 包含分类规则、优先级排序方法、核心需求提取标准、用户故事、原型设计 |
| S5-A01~S5-A06 | 包含架构视图定义、接口规范、部署策略 |
| S6-A01~S6-A04 | 包含模块设计、数据库设计、UI/UX设计、测试策略 |
...
| 维度 | 关键检查点 |
|------|-----------|
| 完整性 | 风险分类完整、可行性维度完整、选型标准完整、非功能需求完整 |
...
| 维度 | 关键检查点 |
|------|-----------|
| 完整性 | 分类规则完整、优先级方法完整、核心提取标准完整、用户故事完整、原型设计完整 |
| 准确性 | 分类正确、优先级排序合理、核心需求提取准确 |
| 可追溯性 | 与前几阶段产物的关联清晰 |

### S5-A01~S5-A06 - S5阶段（架构设计）

| 维度 | 关键检查点 |
|------|-----------|
| 完整性 | 架构视图完整、接口定义完整、部署策略完整 |
| 准确性 | 架构决策合理、技术方案可行 |
| 一致性 | 与需求阶段产物的一致性 |

### S6-A01~S6-A04 - S6阶段（详细设计）

| 维度 | 关键检查点 |
|------|-----------|
| 完整性 | 模块设计完整、数据库设计完整、UI/UX设计完整 |
| 准确性 | 设计细节准确、测试覆盖全面 |
| 可追溯性 | 与架构设计的关联清晰 |
...
**文档版本**: v1.0
**适用对象**: 所有27个Skill（26个工作流Skill + 1个变更管理Skill）
**最后更新**: 2026-04-14
