...
| 竞品分析报告    | `database/stages/s2/{PlanID}/{PlanID}-S2-S09-001-MD.md` | Skill9 产物，竞品分析结果 | 必需  |
...
| 输出类型     | 文件路径                                                    | 产物 ID                    | 说明              |
| -------- | ------------------------------------------------------- | ------------------------ | --------------- |
| 市场痛点验证报告 | `database/stages/s2/{PlanID}/{PlanID}-S2-S10-001-MD.md` | `{PlanID}-S2-S10-001-MD` | Markdown 格式验证报告 |
| 状态更新     | `database/plans/{PlanID}-todo-list.md`                  | -                        | Todo-List 状态更新  |
...
flowchart TD
    ...
    Step7 --> Step8[步骤 8: 用户评审]
    Step8 --> Step9[步骤 9: Todo-List 更新]
    Step9 --> End[结束]
```
...
1. 检查 Skill4 产物是否存在（需求验证报告）
2. 检查 Skill9 产物是否存在（竞品分析报告）
3. 验证 Plan 定义文件是否存在
4. 读取并解析所有输入文件
...
**输出**：`{PlanID}-S2-S10-001-MD.md`

#### 步骤 8：用户评审

**目标**：触发用户评审，确认验证结果。

**评审触发时机**：生成验证报告并通过后置校验后。
...
```markdown
【需求分析-S2 阶段 - 市场痛点验证】- 用户评审

市场痛点验证已完成，输出产物如下：

| 产物 ID | 产物名称 | 格式 | 说明 |
|--------|---------|------|------|
| {PlanID}-S2-S10-001-MD | 市场痛点验证报告 | Markdown | 包含痛点清单、真实性验证、市场价值评估等 |

---
核心发现摘要：
- 识别痛点：{X} 个（显性{X}个，隐性{Y}个，竞品未解决{Z}个）
- 已验证痛点：{N} 个
- 高价值痛点：{M} 个
- 处理建议：继续推进{A}个，需要调整{B}个
...
| ------------------------ | -------- | -------- | ------------------------------------------------------- | --- |
| `{PlanID}-S2-S10-001-MD` | 市场痛点验证报告 | Markdown | `database/stages/s2/{PlanID}/{PlanID}-S2-S10-001-MD.md` | 必需  |

### 4.2 产物 ID 格式

**格式**：`{PlanID}-S2-S10-001-MD`
...
└── stages/
    └── s2/
        └── {PlanID}/
            └── {PlanID}-S2-S10-001-MD.md    # 市场痛点验证报告
```
...
| W301 | 评审超时    | 超过 24 小时未评审  | 状态设为 `paused`，等待用户恢复 |
