...
**示例 3：附带补充材料**
```
边界界定报告：database/stages/s1/P000003-S1-S01-001.md
用户原始需求："我想开发一个知识付费平台"
补充材料：
- 原型图：知识付费平台原型.fig
- 竞品文档：竞品功能对比.xlsx
...
Skill2 完成后，系统会生成以下产物并保存到项目目录：

**产物清单**：

| 产物名称 | 文件位置 | 格式 | 用途 | 用户可见性 |
|---------|---------|------|------|-----------|
| 显性需求报告 | `database/stages/s1/{PlanID}-S1-S02-001.md` | Markdown | 5 类需求清单详情 | ✅ 用户可查看 |
| Todo-List 更新 | `database/plans/{PlanID}/todo-list.md` | Markdown | 更新 S1-S02 任务状态 | ✅ 用户可查看 |
...
2. **Todo-List 状态更新**
   - S1-S02 任务状态：待执行 → 已完成
   - 评审状态：待评审
   - 下阶段准备：S1-S03 隐性需求挖掘（常规模式）或 S1-S04 需求验证（轻量化模式）
...
```mermaid
flowchart TD
    Start[开始执行 Skill2] --> PreCheck[前置校验]
    ...
    GapIdentify --> GenReport[生成显性需求报告]
    ...
    ReviewResult -->|大修改 | ReExecute[重新执行 Skill2]
...
- S1-S01 未完成：返回错误码 E004，提示"S1-S01 任务未完成，请先执行"
...
- 读取用户原始需求文本
- 读取 Todo-List，确认 S1-S02 任务状态为"待执行"
...
**评审展示**：
向用户展示以下内容：
- 显性需求报告核心内容（需求统计、P0 需求清单）
- 需求优先级分布
- 需求缺口分析
- Todo-List 更新状态

**用户决策选项**：
- **确认**：需求提取无误，继续执行 S1-S03
- **修改（小修改）**：直接修改显性需求报告
- **修改（大修改）**：返回步骤 1 重新执行
- **新增想法**：补充需求信息，更新报告
...
| 用户决策 | 处理逻辑 | 状态变更 |
|---------|---------|---------|
| 确认 | 保存产物，更新 Todo-List，进入 S1-S03 | 任务状态：已评审，评审状态：已通过 |
| 修改 - 小修改 | 直接修改产物，重新展示评审 | 任务状态：执行中，评审状态：需修改 |
| 修改 - 大修改 | 返回步骤 1 重新执行 | 任务状态：执行中，评审状态：需重做 |
| 新增想法 | 更新显性需求报告，重新展示评审 | 任务状态：执行中，评审状态：需修改 |
...
| 执行阶段 | 更新时机 | 更新内容 |
|---------|---------|---------|
| Skill 执行开始 | 步骤 1 完成后 | S1-S02 任务状态：待执行 → 执行中 |
| 用户交互后 | 步骤 2.5 完成后 | 更新需求信息（如有补充） |
| Skill 执行完成 | 步骤 13 完成后 | S1-S02 任务状态：执行中 → 已完成，评审状态：待评审 |
...
| S1-S01 未完成 | E004 | 返回错误，要求先完成 S1-S01 | "S1-S01 任务未完成，请先执行" |

#### 执行过程错误

| 错误类型 | 错误码 | 处理策略 | 重试次数 |
|---------|-------|---------|---------|
| 需求识别冲突 | E101 | 标记冲突，继续执行 | 不重试 |
| 需求分类模糊 | E102 | 标记为待确认，继续执行 | 不重试 |
...
- 用途：获取产品边界、用户边界、场景边界等上下文框架

### 3.2 后置 Skill

**常规模式**：
- S1-S03 (隐性需求挖掘)：依赖 Skill2 的显性需求清单
- S1-S04 (需求验证)：依赖 Skill2 的显性需求清单
...
| `{PlanID}-S0-S00-001-todo` | Todo-List | 确认 Skill0 和 Skill1 已完成，更新 S1-S02 状态 |

### 3.4 被依赖的产物 ID

**Skill2 生成的产物被后续 Skill 依赖**：

| 产物 ID | 产物类型 | 被依赖的 Skill | 用途 |
|--------|---------|----------------|------|
| `{PlanID}-S1-S02-001` | 显性需求报告 | S1-S03, S1-S04 | 参考显性需求清单 |
...
**Skill2 产物 ID 示例**：
- `P000001-S1-S02-001` - 显性需求报告（主产物）

**说明**：
- PlanID：6 位数字，如 P000001
- Stage：阶段编号，S1
- SkillID：Skill 编号，S02
- 序号：3 位数字，从 001 开始
...
- 显性需求报告：`database/stages/s1/{PlanID}-S1-S02-001.md`
