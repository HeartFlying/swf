...
本模板用于 Skill4(需求验证) 的输出产物：**需求验证报告**。

**产物 ID 格式**：`{PlanID}-S1-S04-001-validation`

**存储路径**：`database/stages/s1/{PlanID}/{PlanID}-S1-S04-001-validation.md`
...
|-----|------|
| **Plan ID** | {PlanID} |
| **报告 ID** | {PlanID}-S1-S04-001-validation |
...
| **阶段名称** | S1 - 需求边界与原始采集 |
| **执行时间** | {开始时间} - {结束时间} |
| **执行 Skill** | S01(边界界定), S02(显性需求), S03(隐性需求，常规模式), S04(验证) |
| **阶段状态** | ✅ 已完成 |
| **产物数量** | {N} 个 |
| **需求总数** | {N} 个 |
| **验证通过** | {N} 个 (占比{XX}%) |
...
|:-------:|----------|----------|----------|:----:|
| {PlanID}-S1-S01-001 | 需求边界界定报告 | boundary | database/stages/s1/{PlanID}/ | ✅ |
| {PlanID}-S1-S02-001 | 显性需求提取报告 | explicit | database/stages/s1/{PlanID}/ | ✅ |
| {PlanID}-S1-S03-001 | 隐性需求挖掘报告 | implicit | database/stages/s1/{PlanID}/ | ✅ |
| {PlanID}-S1-S04-001 | 需求验证报告 | validation | database/stages/s1/{PlanID}/ | ✅ |
...
| 风险已识别 | ✅ | 主要风险已记录，共{N}个 |
| 状态文件已更新 | ✅ | status.json 已更新，S1 阶段标记为 completed |
