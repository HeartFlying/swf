...
本文档包含 S204 需求验证 Skill 的详细执行步骤说明，供需要深入了解执行细节时参考。
...
- 读取 Todo-List，确认 S204 任务状态为"待执行"
...
| P001-S1-S203-001 | 隐性需求挖掘报告 | implicit   | artifacts/stages/s1/P001/ |  通过  |
| P001-S1-S204-001 | 需求验证报告   | validation | artifacts/stages/s1/P001/ |  通过  |
...
| 执行阶段       | 更新时机       | 更新内容                           |
| ---------- | ---------- | ------------------------------ |
| Skill 执行开始 | 步骤 1 完成后   | S204 任务状态：待执行 → 执行中          |
| 用户交互后      | 步骤 2.5 完成后 | 更新需求信息（如有补充）                   |
| Skill 执行完成 | 步骤 12 完成后  | S204 任务状态：执行中 → 已完成，评审状态：待评审 |
