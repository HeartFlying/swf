...
本模板用于 S204(需求验证) 的输出产物：**需求验证报告**。

**产物 ID 格式**：`{PlanID}-S2-S204-001-validation`

**存储路径**：`artifacts/stages/s2/{PlanID}/{PlanID}-S2-S204-001-validation.md`
...
| 12 | S2 阶段总结 | 阶段执行概况、产物清单、关键结论 | 必需 |
...
|-----|------|
| **Plan ID** | {PlanID} |
| **报告 ID** | {PlanID}-S2-S204-001-validation |
...
| **阶段名称** | S2 - 需求定义 |
| **执行时间** | {开始时间} - {结束时间} |
| **执行 Skill** | S201(边界界定), S202(显性需求), S203(隐性需求，常规模式), S204(验证) |
| **阶段状态** | ✅ 已完成 |
| **产物数量** | {N} 个 |
| **需求总数** | {N} 个 |
| **验证通过** | {N} 个 (占比{XX}%) |
...
|:-------:|----------|----------|----------|:----:|
| {PlanID}-S2-S201-001 | 需求边界界定报告 | boundary | artifacts/stages/s2/{PlanID}/ | ✅ |
| {PlanID}-S2-S202-001 | 显性需求提取报告 | explicit | artifacts/stages/s2/{PlanID}/ | ✅ |
| {PlanID}-S2-S203-001 | 隐性需求挖掘报告 | implicit | artifacts/stages/s2/{PlanID}/ | ✅ |
| {PlanID}-S2-S204-001 | 需求验证报告 | validation | artifacts/stages/s2/{PlanID}/ | ✅ |
| {PlanID}-S2-summary | S2 阶段总结报告 | summary | artifacts/stages/s2/{PlanID}/ | ✅ |
...
| 准备项 | 状态 | 说明 |
|--------|:----:|------|
| S2 阶段产物已归档 | ✅ | 所有产物已保存到 artifacts/stages/s2/{PlanID}/ |
| 需求已验证通过 | ✅ | 核心需求已通过验证，综合得分{分} |
| 风险已识别 | ✅ | 主要风险已记录，共{N}个 |
| 状态文件已更新 | ✅ | Todo-List 已更新，S2 阶段标记为已完成 |
| S3 阶段输入已准备 | ✅ | S2 产物可作为 S3 阶段输入 |
...
- 产物 ID 格式：`{PlanID}-S2-S204-001-{类型}`
