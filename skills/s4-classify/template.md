...
| 数据来源 | 提取需求数 | 占比 | 覆盖度评价 |
|---------|-----------|------|-----------|
| 显性需求提取（S102） | {N} | {X}% | 充分/一般/不足 |
| 隐性需求挖掘（S103） | {N} | {X}% | 充分/一般/不足 |
| 需求验证（S104） | {N} | {X}% | 充分/一般/不足 |
| 市场痛点验证（S202） | {N} | {X}% | 充分/一般/不足 |
| 技术可行性评估（S302） | {N} | {X}% | 充分/一般/不足 |
...
1. **执行 S402**: 基于分类结果进行需求优先级排序
   - 输入：本分类报告
   - 输出：需求优先级排序报告
   - 预计工作量：{X}人天

2. **执行 S403**: 提炼核心需求，形成最终需求规格
   - 输入：优先级排序结果
   - 输出：核心需求清单
   - 预计工作量：{X}人天
