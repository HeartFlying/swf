...
1. 前置校验 → 检查依赖文件（跳过 S103 检查）
2. 读取需求报告 → 提取边界报告和显性需求报告
3. 完整性验证 → 得分 90/100（信息完整度高）
4. 一致性验证 → 得分 95/100（无冲突）
5. 可行性验证 → 得分 82/100（可行性良好）
6. 价值验证 → 得分 90/100（价值明确）
7. 需求分类 → 通过 28 个，有条件通过 5 个，未通过 2 个
8. 风险识别 → 识别 2 个风险（均为中低风险）
9. 生成验证报告 → 按模板生成完整报告
10. 生成阶段总结 → 汇总 S1 阶段产物（不含隐性需求报告）
11. 更新 Todo-List → S104 状态更新为"已完成"
12. 后置校验 → 所有产物格式正确
13. 用户评审 → 等待用户确认
...
- 产物 ID: P000002-S1-S104-001
- 验证得分: 完整性 90/100，一致性 95/100，可行性 82/100，价值 90/100
- 需求分类: 通过 28 个，有条件通过 5 个，未通过 2 个
- 风险数量: 2 个（0 高 2 中 0 低）
- S1 阶段状态: 已完成
...
- `P000001-S1-S104-001` - 需求验证报告（主产物）
...
└── stages/
    └── s1/
        ...
        ├── {PlanID}-S1-S103-001.md     # 隐性需求报告（常规模式）
        ├── {PlanID}-S1-S104-001.md     # 需求验证报告
...
- 需求验证报告：`database/stages/s1/{PlanID}-S1-S104-001.md`
...
| 错误码 | 错误类型 | 说明 | 处理策略 |
|--------|----------|------|----------|
| E001 | 边界报告不存在 | S101 产物未找到 | 返回错误，要求先执行 S101 |
| E002 | 显性需求报告不存在 | S102 产物未找到 | 返回错误，要求先执行 S102 |
| E003 | 隐性需求报告不存在（常规模式） | S103 产物未找到 | 返回错误，要求先执行 S103 |
| E004 | Todo-List 文件不存在 | 状态文件未找到 | 返回错误，要求初始化 Todo-List |
...
| E105 | 文件写入失败 | 磁盘或权限问题 | 重试 3 次，失败则返回错误 |
...
| E204 | Todo-List 结构错误 | 状态更新失败 | 重新更新 Todo-List |
...
| 错误码 | 错误类型 | 说明 | 处理策略 |
|--------|----------|------|----------|
| W401 | 用户交互超时 | 超过等待时间 | 使用默认值继续，标记信息缺失 |
