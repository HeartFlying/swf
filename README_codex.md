...
[![AI IDE](https://img.shields.io/badge/AI%20IDE-Claude%20Code%20%7C%20Codex%20%7C%20Trae-orange.svg)](#-快速开始)
...
当前仓库已补充 **Codex 适配层**：
- `AGENTS.md` 定义 Codex 在本仓库中的执行约定
- `agents/*.md` 继续作为协调器剧本
- `skills/*/SKILL.md` 继续作为逐步加载的工作流定义
- 不依赖 Claude 的自动 Agent 触发机制，改为由 Codex 显式读取和执行
...
- Codex、Claude Code 或 Trae 等 AI IDE
- 将仓库克隆到本地
...
```text
请按 AGENTS.md 执行 SWF 需求分析流程，帮我分析这个需求：我想开发一个在线教育平台。
```

也可以直接描述你的需求，由 Codex 按 `AGENTS.md` 和 `agents/` / `skills/` 约定执行：
...
3. 按需读取当前 Skill 的 `SKILL.md` 和必要引用
4. 生成 `artifacts/` 中的产物并等待用户评审
5. S4 完成后继续执行 S5，S5 完成后继续执行 S6
...
| **轻量化模式** | 评分 ≥ 90 | 21 个 | 快速输出，跳过 5 个 Skill |
...
| [AGENTS.md](AGENTS.md) | Codex 项目指引 |
